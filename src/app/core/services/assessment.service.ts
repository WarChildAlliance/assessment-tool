...
export class AssessmentService {
    ...
    this.getAssessmentsDeep().subscribe(async (assessments) => {
      ...
      for (const assessment of assessments) {
        ...
        for (const topic of assessment.topics) {
          ...
          promises.push(this.answerService
            .getCompleteStudentAnswersForTopic(topic.id)
            .toPromise()
            .then((answers) => (topic.isFirstTry = answers.length === 0)));
          for (const question of topic.questions) {
            // Disable TTS for the moment
            ...
            if (question.question_type === QuestionTypeEnum.DragAndDrop) {
              const bgImage = question.attachments.find((e) => e.attachment_type === 'IMAGE' && e.background_image);
              this.getAttachments([bgImage]);

              this.getQuestionDraggableOptions(assessment.id, topic.id, question.id).subscribe((res: DraggableOption[]) => {
  ...
  public getTutorial(): Observable<Assessment> {
    return this.storedAssessments.pipe(map((assessmentsList) => assessmentsList.find((a) => a.subject === 'TUTORIAL')));
  }

  public getAssessments(): Observable<Assessment[]> {
    return this.storedAssessments.pipe(map((assessmentsList) => this.sortAssessments(assessmentsList)));
  }
  ...
  public sortAssessments(assessmentsList): Assessment[] {
    ...
    mutatedAssessmentList.map((assessment) => {
      if (assessment.subject === 'TUTORIAL') {
        assessment.order = 0;
      }
      if (assessment.subject === 'PRESEL') {
        assessment.order = 1;
      }
      if (assessment.subject === 'POSTSEL') {
        assessment.order = assessmentsList.length;
      }
  ...
  public getAssessment(assessmentId: number): Observable<Assessment> {
    return this.storedAssessments.pipe(map((assessments) => assessments.find((assessment) => assessment.id === assessmentId)));
  }

  public getAssessmentTopics(assessmentId: number): Observable<Topic[]> {
    return this.storedAssessments.pipe(map((assessments) => assessments.find((assessment) => assessment.id === assessmentId).topics));
  }

  public getAssessmentTopic(assessmentId: number, topicId: number): Observable<Topic> {
    return this.storedAssessments.pipe(
      map((assessments) => assessments.find((assessment) => assessment.id === assessmentId).topics.find((topic) => topic.id === topicId))
    );
  }
  ...
  public getAssessmentTopicQuestions(assessmentId: number, topicId: number): Observable<GeneralQuestion[]> {
    return this.storedAssessments.pipe(
      map(
        (assessments) =>
          assessments.find((assessment) => assessment.id === assessmentId).topics.find((topic) => topic.id === topicId).questions
      )
    );
  }
  ...
  private getAttachments(attachments: Attachment[]): void {
    ...
    for (const attachment of attachments) {
      const path = attachment.file.includes(environment.API_URL) ? attachment.file : environment.API_URL + attachment.file;
  ...
  private getQuestionDraggableOptions(assessmentId: number, topicId: number, questionId: number): Observable<any> {
    return this.http.get(`${environment.API_URL}/assessments/${assessmentId}/topics/${topicId}/questions/${questionId}/draggable/`);
  }
}
